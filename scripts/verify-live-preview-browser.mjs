...
async function main() {
  ...
  try {
    await waitForServer(`${baseUrl}/scripts/live-preview-browser-fixture.html`, output);
    ...
    await page.goto(`${baseUrl}/scripts/live-preview-browser-fixture.html`, {
      waitUntil: "networkidle",
    });
    const result = await page.evaluate(() => window.__graniteLivePreviewResult);
    if (!result?.ok) {
      throw new Error(
        `Live Preview browser verification failed:\n${JSON.stringify(result, null, 2)}\n${consoleMessages.join("\n")}`,
      );
    }
    console.log("Live Preview browser verification passed.");
