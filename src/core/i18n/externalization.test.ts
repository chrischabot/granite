...
const SETTINGS_MODAL_FORBIDDEN_PATTERNS = [
  ...
  />\s*Spellcheck\s*</,
  />\s*Spellcheck languages\s*</,
  /placeholder="en-US, he, fr-CA"/,
  />\s*Live preview\s*</,
...
const BASES_VIEW_FORBIDDEN_PATTERNS = [
  ...
  /\$\{config\.filter \? ` · <code class="bases-fence-filter">/,
  /filter: <code>/,
  /"no filter"/,
...
describe("UI string externalization audit", () => {
  ...
  it("keeps Settings modal labels and built-in section titles routed through i18n keys", () => {
    ...
    for (const requiredKey of [
      ...
      "settings.editor.defaultEditingMode",
      "settings.editor.mode.livePreview",
      "settings.editor.spellcheckLanguages",
      "settings.editor.spellcheckLanguagesPlaceholder",
