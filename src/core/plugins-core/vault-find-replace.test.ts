...
describe("replaceInText — literal mode", () => {
  it("replaces every occurrence", () => {
    const { text, count } = replaceInText("hello hello hello", "hello", "hi", {
      caseSensitive: true,
      regex: false,
    });
  ...
  it("case-sensitive misses different cases", () => {
    const { text, count } = replaceInText("Hello hello", "Hello", "hi", {
      caseSensitive: true,
      regex: false,
    });
  ...
  it("escapes regex metacharacters in literal mode", () => {
    const { text, count } = replaceInText("a.b a.b a?b", "a.b", "x", {
      caseSensitive: true,
      regex: false,
    });
  ...
  it("preserves literal `$` characters in the replacement", () => {
    const { text } = replaceInText("price: 100", "100", "$100", {
      caseSensitive: true,
      regex: false,
    });
  ...
  it("preserves literal `$1` in the replacement (no expansion in literal mode)", () => {
    const { text } = replaceInText("abc", "abc", "$1foo", { caseSensitive: true, regex: false });
  ...
  it("treats empty find as no-op", () => {
    expect(replaceInText("hello", "", "x", { caseSensitive: true, regex: false })).toEqual({
      text: "hello",
      count: 0,
    });
  });
...
describe("replaceInText — regex mode", () => {
  it("treats find as a regex pattern", () => {
    const { text, count } = replaceInText("foo123 bar45", "[a-z]+\\d+", "X", {
      caseSensitive: true,
      regex: true,
    });
  ...
  it("respects case-insensitive flag", () => {
    const { text, count } = replaceInText("Foo foo FOO", "foo", "x", {
      caseSensitive: false,
      regex: true,
    });
  ...
  it("expands numbered capture groups with $1/$2", () => {
    const { text, count } = replaceInText("foo123 bar456", "([a-z]+)(\\d+)", "$1-$2", {
      caseSensitive: true,
      regex: true,
    });
  ...
  it("supports the `$&` whole-match token", () => {
    const { text } = replaceInText("abc", "[a-z]+", "[$&]", { caseSensitive: true, regex: true });
